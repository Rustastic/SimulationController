...
impl SimulationController {
    pub fn send_re_init_flooding(&mut self) {
        ...
        for (media_client, (_, _)) in self.mclients.clone() {
            self.handle_mclient_command(&media_client, MediaClientCommand::InitFlooding);
        }
        for (text_server, (_, _)) in self.text_servers.clone() {
            self.handle_text_command(&text_server, ContentServerCommand::InitFlooding);
        }
        for (media_server, (_, _)) in self.media_servers.clone() {
            self.handle_media_command(&media_server, ContentServerCommand::InitFlooding);
        }
